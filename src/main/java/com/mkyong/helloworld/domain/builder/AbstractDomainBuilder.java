...
public abstract class AbstractDomainBuilder<T extends AbstractDomain, S extends AbstractDomainBuilder<T, S>> {
	 ...
	 *
	 * <p>
	 * {@link AbstractDomainBuilder#build}内でこのビルダに追加された
	...
	/**
	 * {@link AbstractDomainBuilder#build()}内で順次実行されるビルダの設定を定義するインタフェース。
	 *
	 * @param <S>
	 *            設定対象ビルダーの型
	 */
	public static interface BuilderConfigurator<S> {
		...
		/**
		 * {@link AbstractDomainBuilder#build()}内で呼ばれる際に実行するビルドアクションを定義する。
		 *
		 * @param builder
		 *            ビルダーインスタンス
		 */
